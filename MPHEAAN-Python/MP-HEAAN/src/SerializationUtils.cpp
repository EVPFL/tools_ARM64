...
void SerializationUtils::writeKey(Key& key, string path) {
	Key* key_c = new Key(key);
	fstream fout;
	fout.open(path, ios::binary|ios::out);
	fout.write(reinterpret_cast<char*>(key_c->rax), Nnprimes*sizeof(uint64_t));
	fout.write(reinterpret_cast<char*>(key_c->rbx), Nnprimes*sizeof(uint64_t));
	fout.close();
	delete key_c;
}
