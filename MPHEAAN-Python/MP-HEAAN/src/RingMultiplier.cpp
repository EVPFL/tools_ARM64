...
void RingMultiplier::mult(uint64_t* rx, ZZ* a, ZZ* b, long np) {
	uint64_t* ra = new uint64_t[np << logN]();
	uint64_t* rb = new uint64_t[np << logN]();
	//uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::multNTT(uint64_t* rx, ZZ* a, uint64_t* rb, long np) {
	uint64_t* ra = new uint64_t[np << logN]();
	//uint64_t* rx = new uint64_t[np << logN]();
	NTL_EXEC_RANGE(np, first, last);
	for (long i = first; i < last; ++i) {
...
void RingMultiplier::multDNTT(uint64_t* rx, uint64_t* ra, uint64_t* rb, long np) {
	NTL_EXEC_RANGE(np, first, last);
	for (long i = first; i < last; ++i) {
		uint64_t* rai = ra + (i << logN);
		uint64_t* rbi = rb + (i << logN);
