...
const long ENCRYPTION = 0;
const long MULTIPLICATION  = 1;
const long CONJUGATION = 2;
const long ROTATION = 3;
...
public:
	...
	Scheme(Ring& ring, bool isSerialized = false);
	...
	void mult(Ciphertext& res, Ciphertext& cipher1, Ciphertext& cipher2, Key& key);

	void multAndEqual(Ciphertext& cipher1, Ciphertext& cipher2);
	void multAndEqual(Ciphertext& cipher1, Ciphertext& cipher2, Key& key);

	void square(Ciphertext& res, Ciphertext& cipher);
	void square(Ciphertext& res, Ciphertext& cipher, Key& key);

	void squareAndEqual(Ciphertext& cipher);
	void squareAndEqual(Ciphertext& cipher, Key& key);
	...
	void leftRotateFastAndEqual(Ciphertext& cipher, long r, Key& key);
	...
	//------------ evaluation method ------------
	void allSum(Ciphertext& ciph_res, Ciphertext& ciph, Key& rot_key, long rot_num);
	void evalAndEqual(Ciphertext& ciph_res, Ciphertext& ciph_x, Ciphertext& ciph_y, Ciphertext& ciph_w, Ciphertext& ciph_b, Key& relin_key, Key& rot_key, long rot_num); 
	
	//------------ mutiple threads ------------
	void intervalSumAndEqual(Ciphertext& ciph, Key& rot_key, long rot_num, int rot_interval);
	static void* intervalSumThread(void *threadarg);

	void allSum(Ciphertext& ciph_res, Ciphertext& ciph, Key& rot_key_1, Key& rot_key_tn, long rot_num, int threads_num);
	void evalAndEqual(Ciphertext& ciph_res, Ciphertext& ciph_x, Ciphertext& ciph_y, Ciphertext& ciph_w, Ciphertext& ciph_b, Key& relin_key, Key& rot_key_1, Key& rot_key_tn, long ele_num, int threads_num);
