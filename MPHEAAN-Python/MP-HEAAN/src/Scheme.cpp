...
Scheme::Scheme(SecretKey& secretKey, Ring& ring, bool isSerialized) : ring(ring), isSerialized(isSerialized) {
	//initKeyMap(keyMap, leftRotKeyMap, serKeyMap, serLeftRotKeyMap);
	addEncKey(secretKey);
	addMultKey(secretKey);
};

Scheme::Scheme(Ring& ring, bool isSerialized) : ring(ring), isSerialized(isSerialized) {
	//initKeyMap(keyMap, leftRotKeyMap, serKeyMap, serLeftRotKeyMap);
};
...
void Scheme::encryptMsg(Ciphertext& cipher, Plaintext& plain) {
	...
	Key* key = isSerialized ? SerializationUtils::readKey(serKeyMap[ENCRYPTION]) : keyMap[ENCRYPTION];
...
void Scheme::mult(Ciphertext& res, Ciphertext& cipher1, Ciphertext& cipher2, Key& key) {
	res.copyParams(cipher1);
	res.logp += cipher2.logp;
...
void Scheme::multAndEqual(Ciphertext& cipher1, Ciphertext& cipher2, Key& key) {
	ZZ q = ring.qpows[cipher1.logq];
	ZZ qQ = ring.qpows[cipher1.logq + logQ];
	long np = ceil((2 + cipher1.logq + cipher2.logq + logN + 2)/(double)pbnd);
	uint64_t* ra1 = new uint64_t[np << logN];
...
void Scheme::square(Ciphertext& res, Ciphertext& cipher, Key& key) {
	res.copyParams(cipher);
	res.logp += cipher.logp;
	ZZ q = ring.qpows[cipher.logq];
	ZZ qQ = ring.qpows[cipher.logq + logQ];
...
void Scheme::squareAndEqual(Ciphertext& cipher, Key& key) {
	ZZ q = ring.qpows[cipher.logq];
	ZZ qQ = ring.qpows[cipher.logq + logQ];
	long np = ceil((2 + 2 * cipher.logq + logN + 2)/(double)pbnd);
	uint64_t* ra = new uint64_t[np << logN];
...
void Scheme::leftRotateFastAndEqual(Ciphertext& cipher, long r, Key& key) {
	ZZ q = ring.qpows[cipher.logq];
	ZZ qQ = ring.qpows[cipher.logq + logQ];
...
void Scheme::allSum(Ciphertext& ciph_res, Ciphertext& ciph, Key& rot_key, long rot_num){
	ciph_res.copy(ciph);

	Ciphertext ciph_rot;
	ciph_rot.copy(ciph);

	for (long i = 0; i<rot_num; i++){
  	leftRotateFastAndEqual(ciph_rot, 1, rot_key);
  	addAndEqual(ciph_res, ciph_rot);
  }
}


void Scheme::evalAndEqual(Ciphertext& ciph_res, Ciphertext& ciph_x, Ciphertext& ciph_y, Ciphertext& ciph_w, Ciphertext& ciph_b, Key& relin_key, Key& rot_key, long rot_num) {
	long logp = ciph_x.logp;
	mult(ciph_res, ciph_w, ciph_x, relin_key);

  Ciphertext ciph_tmp;
  allSum(ciph_tmp, ciph_res, rot_key, rot_num);
  reScaleByAndEqual(ciph_tmp, logp);

  Ciphertext ciph_b_tmp;
  Ciphertext ciph_y_tmp;
  modDownBy(ciph_b_tmp, ciph_b, logp);
  modDownBy(ciph_y_tmp, ciph_y, logp);

  addAndEqual(ciph_tmp, ciph_b_tmp);
  addAndEqual(ciph_tmp, ciph_y_tmp);
  square(ciph_res, ciph_tmp, relin_key);
}


//------------ mutiple threads ------------
struct intervalSumThreadArgs{
	int id;
	long rot_num;
	int rot_interval;
	Scheme* scheme;
...
void Scheme::intervalSumAndEqual(Ciphertext& ciph_rot, Key& rot_key, long rot_num, int rot_interval){
	Ciphertext ciph_res;
	ciph_res.copy(ciph_rot);

	for (long i = 0; i<rot_num; i++){
  	leftRotateFastAndEqual(ciph_rot, rot_interval, rot_key);
  	addAndEqual(ciph_res, ciph_rot);
  }
  ciph_rot.copy(ciph_res);
}

void* Scheme::intervalSumThread(void *threadarg) {
	intervalSumThreadArgs *args = (intervalSumThreadArgs*) threadarg;
  args->scheme->intervalSumAndEqual(args->cipher, args->rot_key, args->rot_num, args->rot_interval);
}


void Scheme::allSum(Ciphertext& ciph_res, Ciphertext& ciph, Key& rot_key_1, Key& rot_key_tn, long rot_num, int thread_num){
  // mutiple threads to compute intervalSum(ciph)
  Ciphertext* ciph_rots = new Ciphertext[thread_num];
  ciph_rots[0].copy(ciph);
	for (long i = 1; i < thread_num; i++){
		ciph_rots[i].copy(ciph_rots[i-1]);
  	leftRotateFastAndEqual(ciph_rots[i], 1, rot_key_1);
  }
  pthread_t threads[thread_num];
	int error;
	intervalSumThreadArgs *tds[thread_num];
	for ( int i=0; i < thread_num; i++ ) {
		intervalSumThreadArgs* td = new intervalSumThreadArgs();
   	td->id = i;	
	  td->rot_num = rot_num/thread_num;	
		td->rot_interval = thread_num;
		td->scheme = this;
		td->cipher = ciph_rots[i];
		td->rot_key = rot_key_tn;
		tds[i] = td;
	}
  for (long i = 0; i < thread_num; i++){
  	error = pthread_create(&threads[i], NULL, intervalSumThread, (void *)tds[i]);
  ...
  // compute sum of intervalSum
  ciph_res.copy(ciph_rots[0]);
  for (long i = 1; i < thread_num; i++){
		addAndEqual(ciph_res, ciph_rots[i]);
  }
...
void Scheme::evalAndEqual(Ciphertext& ciph_res, Ciphertext& ciph_x, Ciphertext& ciph_y, Ciphertext& ciph_w, Ciphertext& ciph_b, Key& relin_key, Key& rot_key_1, Key& rot_key_tn, long ele_num, int thread_num) {
	long logp = ciph_x.logp;
	mult(ciph_res, ciph_w, ciph_x, relin_key);

  Ciphertext ciph_tmp;
  allSum(ciph_tmp, ciph_res, rot_key_1, rot_key_tn, ele_num, thread_num);
  reScaleByAndEqual(ciph_tmp, logp);
