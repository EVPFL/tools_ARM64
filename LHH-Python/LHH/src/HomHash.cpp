...
void _test_HomHash()
{
	...
	for (int i = 0; i < batch; ++i)
	{
		...
		// for (int j = 0; j < d; ++j)
		// 	cout << "my_vec[" << j << "]: " << BN_bn2dec(my_vec[j]) << endl;
		// cout << "alphas[" << i << "]: " << BN_bn2dec(alphas[i]) << endl;
	...
	// for (int j = 0; j < d; ++j){
	// 	cout << "agg_vec[" << j << "]: " << BN_bn2dec(agg_vec[j]) << endl;
	// }
	...
	cout << "ver_res: " << (EC_POINT_cmp(hh.curve, agg_hash, ver_agg_hash, ctx)==0) << endl;
